...
function jsobj2flatten(crystal){
    if (crystal.symops) root.warning("Warning! Reading of symmetry operations is not yet implemented, expect errors now...");
...
var __packer_flag = true;

/* REMBLOCK:START */
var window = window || { alert: function(){} };
var CLIENT = window.prompt;
var SERVER = (typeof module !== 'undefined' && module.exports && !CLIENT);

// Env switch: ***server***
if (SERVER){
    var logger = {warning: console.error, error: console.error};
    var math = require('./math.custom.js'); // for testing, adjust if needed
    math = math.math || math;
    module.exports = MatinfIO(math, logger);
} else if (CLIENT){
...
// Env switch: ***browser bundled unpacked***
    define(['libs/math.custom'], function(math){ // for player.html, must be reflected in build config
        return MatinfIO(math, logger);
    });
    } else if (window.math){

// Env switch: ***browser separated***
        MatinfIO = MatinfIO(window.math, logger);
    } else window.alert("Invalid initialization of the math library!");
}
__packer_flag = false;
/* REMBLOCK:END */
